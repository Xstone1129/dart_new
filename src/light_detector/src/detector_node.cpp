...
DetectorNode::DetectorNode(const rclcpp::NodeOptions &options)
  ...
  img_msg_sub_ = this->create_subscription<sensor_msgs::msg::Image>(
      "/image_raw", 10,
      std::bind(&DetectorNode::targetWorkCallback, this,
                std::placeholders::_1));
}
...
void DetectorNode::targetWorkCallback(
  ...
  target_msg.yaw_error =
      light.is_detected
          ? (cam_center_.y - static_cast<int>(light.center_point.y) +
             compensation)
          : 0;
  ...
  if (debug_) {
    ...
    cv::putText(detector_->debug_image_, yaw_text, cv::Point(20, 40),
...
void DetectorNode::createDebugPublishers() {
  ...
  dilate_img_pub_ =
      image_transport::create_publisher(this, "/detector/dilate_img");
  result_img_pub_ =
      image_transport::create_publisher(this, "/detector/result_img");
}

void DetectorNode::publishDebugImages() {
  binary_img_pub_.publish(cv_bridge::CvImage(std_msgs::msg::Header(), "mono8",
                                             detector_->binary_img_)
                              .toImageMsg());
  dilate_img_pub_.publish(cv_bridge::CvImage(std_msgs::msg::Header(), "mono8",
                                             detector_->dilate_img_)
                              .toImageMsg());
  result_img_pub_.publish(cv_bridge::CvImage(std_msgs::msg::Header(), "bgr8",
                                             detector_->debug_image_)
                              .toImageMsg());
}
